# Science Hub — Development Log (2026-06-21 → 2026-09-07)
...
## Era 15 — Full-app 10-specialist audit + fix pass (2026-09-05)

A second full-app audit, this time run as 10 parallel specialist passes (security, database, performance, api, regression, ui, react, typescript, testing, architect) rather than the sequential 14-phase structure Era 12.4's audit used — weighted toward everything shipped since that audit (notifications, statements, GSTR-1 CSV export, rate-list import, idempotency hardening). 38 findings surfaced, all fixed in the same pass except one left as an open product decision and two confirmed to need no code change. Full finding-by-finding detail (IDs, Location, Problem, Fix) lives in `docs/SCIENCE_HUB_AUDIT_REPORT.md`'s `2026-09-05` entries, folded into that file's existing severity sections; the summary is also in `CLAUDE.md`'s Features Completed #44.

Highlights, grouped by theme rather than the report's per-finding IDs:
- **Security**: the GSTR-1 CSV export (Era 14) hadn't been run through Era 13's `neutralizeFormulaCell()` formula-injection guard — fixed, and the two near-duplicate `Validation-Report.csv` builders (`gstFilingZip.ts`, `gstr1CsvZip.ts`) were merged into one shared `src/lib/validationCsv.ts` so they can't silently diverge on that guard again. Customer/vendor statement routes were gated on bare `requireSession()` instead of the matching `requireSectionAccess()` — a revoked "Payments Received" grant didn't actually block access to a customer's full statement.
- **Idempotency, extended**: `POST /api/invoices`/`/api/purchase-bills` (unlike the Era 12.6 child-resource routes) treated *any* row matching a submitted idempotency key as a safe replay with no parent-match check — a reused/stale key against a different customer/vendor now 409s instead of silently handing back an unrelated document. The customer credit-limit check now runs *inside* the same Serializable transaction as the invoice write (`checkCustomerCreditLimit()` takes a `tx`/`prisma` client as its first arg) — previously a plain pre-check outside the transaction, racy under concurrent invoice creation for the same customer.
- **The BIZ-003 date-boundary bug class recurred** in three newer routes (customer/vendor statement, e-way-bill report) that built date-range filters via raw UTC-midnight parsing instead of `toIstDateStr()`'s IST-aware approach — new `istDayStartUtc()`/`istDayEndUtc()` helpers added to `validation.ts` and applied to all three, alongside 400-on-invalid-range validation they'd also been missing.
- **Performance**: `notifications.ts`'s four exported functions had each re-implemented the same 5-category query/mapping logic (a real correctness risk, not just duplication — a threshold change had 3-4 places to update in sync) — consolidated into shared `fetchOverLimitCustomers()`/`fetchBinExpiringEntities()`/`getActiveDismissalMap()` helpers, which also closed an unbounded credit-limit query and turned 5 near-identical dismissal lookups into one. Statement queries (`statementQuery.ts`) no longer fetch a customer/vendor's entire lifetime history to compute a filtered range — the date range now pushes into the Prisma `where`, with everything older collapsed into an `openingBalanceSeed` via three SUM aggregates (`buildLedger()` gained that as an optional, backward-compatible 4th param).
- **Accessibility, in the topbar popover work that was still uncommitted at audit time**: `AccentPicker`/`MoreMenu` had no Escape/focus-trap, while the new `PopoverScrim` dimmed the whole viewport — a keyboard user who opened either had no way to close it. Fixed by wiring both through the same `useMenuA11y` hook `NotificationBell` already used. Separately, `PopoverScrim` itself only *dimmed* rather than actually blocking clicks (`pointer-events: none`) — a click on a dimmed sidebar link still activated it while the popover also closed. Now `pointer-events: auto` (each popover's outside-click listener still fires correctly, since the scrim was always portaled outside every popover's own ref) with its z-index moved off the mobile sidebar backdrop's own tier.
- **Tests added**: `buildLedger` (incl. the new seed param), the rate-list import parser (header detection, positional fallbacks, quoted-CSV), the statement + notification routes (section-gating, per-user dismissal scoping), and — closing the highest-severity coverage gap — invoice-payment idempotency, including a real concurrent-race case fired via `Promise.all` against a genuine test database transaction, not a mock.
- **Left open, deliberately**: whether a credit note should still list once its parent invoice is in the Bin is a product decision (both are GST-retained document types), not something changed unilaterally.

---

## Era 16 — Product List Price/Purchase Price consistency, shared quick-add modal, credit-note discount inheritance (2026-09-06, `51a0584`, merged to `main`)

A single-day, multi-pass session closing a real divergence bug: a Product's `purchasePrice` could be typed directly *or* reconstructed from `listPrice × (1 − discountPercent/100)`, and once those two disagreed (a one-off vendor concession entered after the fact), a Purchase Bill line item picking that product from the catalog would silently reconstruct the stale discount-based rate instead of trusting the manually-corrected one. Fixing that properly meant making `listPrice` mandatory and `purchasePrice` a pure computed value everywhere it's entered (Product create/edit, bulk import, both quick-add popups), which then surfaced a matching gap on the credit-note side (a return's discount wasn't inherited from its parent invoice line at all) and on GST rate (still a fixed 0/5/12/18/28 dropdown in three different places, blocking any product taxed outside those five brackets). All of it shipped together rather than split into three PRs, since each layer's tests depend on the layer below being fixed first.

- **Product pricing model**: `Product.listPrice` (vendor's pre-discount quoted price) is now a mandatory `Float` column (`discountPercent` stays optional, default 0) — migration `20260906120000_add_product_list_price_discount` added both nullable, `20260906140000_require_product_list_price` backfilled every existing row (`listPrice = COALESCE(purchasePrice, price)`) before locking `listPrice` `NOT NULL`. `purchasePrice` stays a nullable column at the schema level but is enforced mandatory at the app layer and is now a **disabled, computed-only** field in `ProductFormFields.tsx` (`hint="Computed from List Price − Discount %."`) — no longer something a user types into and can silently desync from List Price/Discount %. Selling Price gained a live margin-%-over-Purchase-Price hint (green/amber) via `FormField`'s new `hintWarning` tone, and mirrors Purchase Price's value automatically for as long as it's never been typed into directly (so a List Price/Discount % edit before Selling Price is ever touched keeps both in sync, not just Purchase Price).
- **Purchase Bill and the shared quick-add popup get the identical shape**: `PurchaseBillItemsTable.tsx`'s own items table gained a computed read-only "Rate (₹)" (net, post-discount) column, and picking a catalog product (`resolvePurchaseBillLineRate()`) or the quick-add popup (`resolveQuickAddLineRate()`) both now trust an already-diverged Purchase Price over reconstructing it from List Price × Discount % — the exact bug this whole pass started from. Both helpers live in `src/lib/purchaseBillForm.ts` for unit-testability.
- **`QuickAddProductModal.tsx`** (`src/components/products/`) replaces what had been two near-duplicate "Add Custom Item" popups hand-rolled inside `InvoiceLineItemsCard.tsx` and `PurchaseBillItemsTable.tsx` (dating to 2026-08-11) with one shared component — each caller lost 150-250 lines of duplicate modal JSX/validation/submit-to-`/api/products` logic and now just maps the popup's `QuickAddOutcome` onto its own line-item shape. Fed by a new `GET /api/units` + `useUnitSuggestions()` hook (merges a form's static unit list with every distinct unit already saved across Product/InvoiceItem/PurchaseBillItem), also wired into `ProductFormFields.tsx`'s own Unit field.
- **GST rate freed from its fixed dropdown**: Product/Invoice/PurchaseBill line-item `gstRate` moves from a `<Select>` over `[0,5,12,18,28]` to free-text validated `0–100` (`rules.percentRange()` client-side, `validateNumericField(...,{max:100})`/inline range checks server-side) — a real product (precious stones/gold at 0.25% or 3%) is now representable. Invoice's own line-items table also gained inline-editable Unit and GST % cells (previously read-only badges) to match.
- **Credit-note (`Return`) discount inheritance**: `ReturnItem` gained `discountPercent`/`discountAmount` (migration `20260906130000_add_return_item_discount`), and `POST /api/invoices/[id]/returns` now inherits a line's discount from the matching original invoice line by `productId` (`discountByProduct` map, mirroring the existing GST-rate-inheritance `rateByProduct` map) rather than trusting the client — closing a real over-crediting bug where a return against a discounted invoice line was computed at the pre-discount gross price. A client-supplied `discountPercent` is only used as a fallback for a custom, non-catalog return line with no `productId` to trace back to.
- **Sales Invoice line-item pricing prefill, reversed then re-applied same day**: an earlier attempt this same day to prefill a new invoice line from a product's List Price/Discount % (instead of its flat Selling Price) was tried, reverted as purely cosmetic risk (see the "Do not" rule in `CLAUDE.md` for the original reasoning), then re-requested and re-applied once that tradeoff was explicitly re-confirmed acceptable — `InvoiceLineItemsCard.tsx`'s `addProduct()` now prefills List Price + Discount % whenever Selling Price still exactly equals Purchase Price (the app's own proxy for "never deliberately priced away from cost"), and leaves an already-diverged Selling Price alone. The at/below-cost warning pill now compares the post-discount net rate rather than the raw List Price.
- **Smaller fixes bundled into the same pass**: `POST /api/invoices` had `unit: product?.unit || item.unit` — the catalog product's own unit silently overrode a client-supplied custom unit, backwards from every other override-precedence rule in the app; flipped to `item.unit || product?.unit || "Nos"`. Invoice/PurchaseBill item routes (create, edit, and invoice returns) now cap `price`/`purchasePrice` at `MAX_MONEY_VALUE` and reject a blank `unit`, closing the same `"Infinity"`-string gap Product routes had already closed. Global search (`GET /api/search`) now also matches an invoice/purchase-bill's line-item names, not just its own number or party name. `TaxonomyDetailPage.tsx`'s Brand/Category product-table column relabeled "List Price" → "Selling Price" now that "List Price" means something else on `Product`.
- **Tests added**: `tests/api/products.test.ts` (new — mandatory `listPrice`/`purchasePrice`, discount/GST-rate range checks, `Infinity` rejection, edit-time re-validation), `tests/unit/productForm.test.ts` (new — `validateProductForm`'s `listPriceRequired` opt-out, `resolveSellingPrice()`), `tests/unit/productImport.test.ts` (new — header detection across List Price/Discount %/Selling Price, positional fallback, skip-on-missing-List-Price), plus new cases in `tests/api/invoices.test.ts` (GST-rate range), `tests/api/purchase-bills.test.ts`, `tests/api/returns.test.ts` (discount-inheritance), and `tests/unit/purchaseBillForm.test.ts`/`validation.test.ts` (`rules.percentRange()`, the new rate-resolution helpers).

### 16.1 — `aaefb0a`/`b4b6349`: two independent-review follow-ups, same day

`51a0584` above shipped, then got a second look from independent review before the day was out, surfacing two real regressions and one process gap:
- **`aaefb0a`**: Product Edit's now-computed Purchase Price field rendered permanently blank (with the Update button silently locked, no visible reason why) for the handful of legacy rows whose `purchasePrice` predates the List Price/Discount % pair and was still stored `null` — the computed-display logic had no fallback path for that case. Fixed by recomputing Purchase Price from List Price × Discount % whenever the stored value is `null`, same as new rows already got. Also fixed a test that still asserted the pre-`51a0584` behavior ("clearing List Price sets it to null"), which the mandatory-List-Price change had made obsolete but nobody had gone back to update.
- **`b4b6349`**: Invoice New/Edit's line-item validation loop had no "every item must have a unit" check — Purchase Bill already validated this, but Invoice's Unit column had only just become editable in `51a0584` and the equivalent guard was never ported over. Also: `/api/setup` (the dev-only demo-seed endpoint) wasn't setting `purchasePrice` on its demo products, reproducing on every fresh dev setup the exact null-`purchasePrice`-with-no-List-Price edge case `aaefb0a` had just fixed for real legacy data.

Worth remembering as a pattern, not just a one-off: **a mandatory-field/computed-value migration needs its fallback path audited for every row that predates the migration**, not just verified against freshly-created rows — the gap in both `aaefb0a`'s bug and the null-`purchasePrice` edge case it fixed is the same shape.

---

## Era 17 — Item quantity precision, app-wide IST/idempotency/build-reliability pass, shared payment-method list, document-number zero-padding (2026-09-06)

Four same-day commits, landed in this order, none large enough alone to warrant its own top-level era but collectively a full day's worth of hardening:

### 17.1 — `8dc5eb4`: item quantity capped to 3 decimal places

Invoice/purchase-bill line-item quantity inputs now reject an out-of-range keystroke outright, matching the draft-buffer validation pattern their Discount %/GST % inputs already used (type past 3 decimals and the keystroke is simply refused, not silently truncated after the fact on blur/submit).

### 17.2 — `8f78b91`: the IST/UTC date-boundary audit (folded into `CLAUDE.md` Features Completed #48)

Triggered by a real, small symptom — a credit note appearing out of "newest first" order — that turned into an app-wide audit once the root cause (raw UTC-midnight boundaries used where an IST calendar-day boundary was needed) turned out to recur in several unrelated places: `getGstSummary`/`getReportOutstanding`/`getPurchaseOutstanding` were silently excluding same-day invoices/bills from Sales/Purchase Reports on a server running UTC (this dev machine's IST clock had been masking it locally the whole time); `getIndianFinancialYear()` could number a document into the wrong financial year in the ~5.5-hour gap around 1 April IST midnight; the same bug pattern had also reached the GSTR-1 CSV export's date formatting, three dashboard "this month" cards, and the notifications module's due/overdue bucketing. Fixed via a family of new IST-aware helpers in `src/lib/validation.ts` (`istTodayStartUtc`, `istMonthStartUtc`, `istNextMonthStartUtc`, `istMonthBoundsUtc`, alongside the pre-existing `istDayStartUtc`/`istDayEndUtc`/`toIstDateStr`) and a hardening pass normalizing every `Payment`/`Return`/`PurchasePayment` date to exact IST midnight at write time, closing the sort-order bug at its root rather than just the symptom.

Bundled in the same commit: **Customer/Vendor/RateList create gained the idempotency-key protection** every other create route already had (`Customer.idempotencyKey`/`Vendor.idempotencyKey`/`RateList.idempotencyKey`, migration `20260906150000_add_customer_vendor_ratelist_idempotency_key`), wired into all 6 real call sites (3 standalone New pages + 3 persistently-mounted inline "Add New" forms). Smaller fixes riding along: `Modal`'s initial-focus effect always stole focus to the header close button instead of an intended `autoFocus` field (silently defeating every `autoFocus` used inside a Modal body app-wide); `FormField` dropped `aria-describedby` injection whenever a caller passed an explicit `id`; Rate List delete never busted its own list cache; a product-search `.filter()` re-scanned the entire catalog on every unrelated line-item keystroke; and — a second, unrelated build-reliability finding from the same stress-testing pass that found the `.next/dev` issue back in Era 6 — re-running `next build` against an *already-successfully-built* `.next` folder could intermittently corrupt Turbopack's generated route-validator file mid-write and fail the build with a `TS1109`/`TS1128` syntax error, a genuinely racy concurrent-write issue (roughly 50% failure rate reproduced directly) rather than anything deterministic in application code. `prebuild` now wipes the whole `.next` folder, not just `.next/dev` — the accepted trade-off is losing Turbopack's incremental cache on every build for reliability. **Every fix in this commit was exercised live against the dev database before being considered done**, not just reviewed, with all test data cleaned up via the app's own delete routes afterward.

### 17.3 — `84e868b`: shared payment-method list (IMPS + free-text "Other"), Notification Bell cache-bust

`PAYMENT_METHODS`/`resolvePaymentMethod()` (`src/lib/paymentMethods.ts`) unifies what had been three separately hand-rolled method lists — the Invoice detail page's Record Payment form, the Purchase Bill detail page's own, and the standalone `RecordPaymentDialog` used by New Purchase Bill's inline payment — so the list (now including `IMPS`) and the "Other reveals a required free-text field" behavior can't drift between them. `Payment.method`/`PurchasePayment.method` are plain free-text DB columns with no enum, so "Other" just persists whatever was typed in its place. Also fixed: the Bin page's restore/permanent-delete/empty-bin actions weren't busting the Notification Bell's cached summary, leaving a stale "days left before auto-purge" count visible for an item that had just been restored or purged.

### 17.4 — `facb5bc`: zero-pad `seq_fy`/`prefix_seq_fy` sequences to 2 digits

Single-digit sequences under these two number-format layouts previously rendered unpadded (`"9/2026-27"`), inconsistent with how the business's own pre-existing manual numbering had looked before adopting this app. Both now zero-pad to 2 digits (`"02/2026-27"`) and grow past that with no cap once the sequence itself needs more digits, matching the padding behavior `prefix_fy_seq` already had. `findMaxSequence()`'s doc comment (`src/lib/documentNumbering.ts`) was stale after this change — it still said these two layouts "don't zero-pad" at all, which stopped being true the moment 2-digit padding was added; corrected in the same session `CLAUDE.md` got its own catch-up pass (see Era 19 below).

---

## Era 18 — Second-wave IST date-picker fixes, purchase-bill attachment size (2026-09-07, `efa52e4`)

A follow-up to Era 17.2's IST audit, this time surfacing in **date-picker min/max validation** rather than report/dashboard aggregation boundaries — a different call-site shape of the same underlying mistake (deriving "today" or a document's own date via a raw UTC operation instead of the IST-aware helpers). Concretely: `.toISOString().slice(0, 10)` and `.split("T")[0]` were still in use across the invoice/purchase-bill payment, return, and edit pages, plus the report and customer/vendor statement pages, to compute a date picker's min/max bound or "is this date in the future" check. The practical symptom: for any document stored at or after 18:30 UTC (an invoice backfilled late at night IST, say), a raw UTC-slice rolls the calendar day over about 5.5 hours before IST midnight actually arrives — so a payment/return date picker on that document could be made to accept (or reject) a date it shouldn't. All the affected pages now go through `toIstDateStr()`/`isFutureIstDate()`, the same helpers Era 17.2 already centralized for the aggregation-boundary class of this bug.

Bundled in the same commit, unrelated: `PurchaseBill.attachmentSize` (nullable `Int`, migration `20260907120000_add_purchase_bill_attachment_size`) — byte size captured at upload time and threaded through the create/edit routes and forms, shown on the bill detail page via a new `src/lib/formatFileSize.ts` helper so the attachment display doesn't need a round-trip to Blob storage just to read `Content-Length`. A one-off, idempotent backfill script (`prisma/backfillAttachmentSize.ts`) populated the column for every attachment uploaded before the column existed.

---

## Era 19 — Payment edit/delete (2026-09-07, uncommitted on `main` as of this entry)

The user's own inspection surfaced a real, longstanding gap: once a Payment or PurchasePayment was recorded, there was no way in the app to correct a mistake in it — not even a UI hook-up existed for the one API route (`PUT /api/invoices/[id]/payment/[paymentId]`) that could already do it, and the purchase-bill side had no edit/delete route at all, only create.

### 19.1 — First pass: the feature itself

Added `DELETE /api/invoices/[id]/payment/[paymentId]` and a brand-new `src/app/api/purchase-bills/[id]/payment/[paymentId]/route.ts` (PUT+DELETE), both mirroring the existing Serializable-transaction-with-P2034-retry pattern every other money-mutating route in this app uses, and both recomputing `paidAmount` from the full set of remaining payment rows (never incrementing/decrementing a stored total) so a stale value can't drift from what actually happened. Both invoice/purchase-bill detail pages' Payment History tables gained an Actions column (`canWrite`-gated edit-pencil/delete-trash icon buttons); Edit opens a fullscreen `Modal`, Delete goes through the standard `ConfirmDialog` — no new UI pattern introduced, both reuse what every other edit/delete flow in the app already does.

**The one genuine new data-integrity risk this feature introduces, found and closed before it shipped**: a credit note is capped against an invoice's `paidAmount` only at the credit note's own creation time (`returns/route.ts`) — that cap is a point-in-time check, not a standing DB constraint, so reducing or deleting a payment afterward could otherwise leave the invoice claiming to have refunded more cash via credit note(s) than it ever actually received. Both the PUT and the new DELETE re-verify `newPaidAmount >= existingReturnTotal` inside the same transaction the payment mutation happens in, and 400 with a clear message if a correction would violate it. A cancelled purchase bill's `status` is preserved verbatim across a payment edit/delete recompute, rather than being silently recomputed away from "cancelled" — mirroring the same "cancelled is a directly-set, not recomputed, status" convention the bill's own edit route already followed.

### 19.2 — Live-verification pass: seeded rows, real browser, real dev database

Rather than stop at unit/API-level tests, this pass drove the actual feature through a real Chromium browser (Playwright, launched ad hoc via a temporary script — not the project's own `tests/e2e/**` harness, which needs its own isolated test database this repo has never had provisioned) logged in as the dev admin, against isolated seeded rows (a test customer/invoice/payment, a fully-paid invoice + credit note to exercise the floor guard, a test vendor/bill/payment, a cancelled bill + payment) clearly marked with a `TEST-PW-`/`ZZZ PLAYWRIGHT TEST` naming convention so they could be identified and hard-deleted afterward with confidence nothing else was touched. All 16 scenario checks passed on the first fully-debugged run (several earlier iterations of the *test script itself* had false negatives — a stale-toast race, an unwrapped-overlay boundingBox measurement, a couple of `waitForTimeout` calls too short for the page's own re-fetch to land — corrected by switching to polling-based waits and role-scoped locators rather than blind sleeps). Confirmed independently via a direct authenticated API call (not just a UI toast) that the DELETE guard's error message is genuinely distinct from the PUT guard's, closing any doubt that a lingering toast from the prior action was being mistaken for proof the delete-side guard fired.

### 19.3 — Third pass: two real gaps the user's own review caught

Live-verification in 19.2 had exercised every *functional* path (edit succeeds, delete succeeds, guards block correctly) but never the *idle/no-op* case — asked directly by the user ("why does Save still fire when nothing changed?"), which turned out to be a genuine, unaddressed gap:

- **No dirty-check on Edit Payment's Save button** — it stayed enabled and would re-PUT identical values, an unnecessary write plus a spurious `ActivityLog` entry, every time the modal was opened and Saved without changing anything. Fixed with `useDirty()` (the same hook Rate List Edit already uses for this exact purpose, Era 10.4) — `openEditPayment()` calls `markClean()` with the row's current values the instant the modal opens, Save is `disabled` when nothing's changed, and the submit handler itself also short-circuits on the same check as defense-in-depth.
- **No `try/catch` around the invoice detail page's three payment `fetch()` calls** (`handleAddPayment`/`handleEditPaymentSubmit`/`handleDeletePayment`) — unlike the purchase-bill detail page's equivalent handlers, which already had one. A genuine network failure (a thrown `fetch` rejection, not just a 4xx/5xx response) would leave the saving/deleting flag stuck `true` forever: the full-page `OverlayLoader` never clears and the button stays disabled with no way out short of a full reload. Traced to `handleAddPayment` (pre-dating this feature — the new Edit/Delete handlers were written by copying its shape, carrying the gap forward), fixed on all three invoice-side payment handlers at once to match the purchase-bill page's already-correct pattern.

Also reduced the Edit Payment modal's width (`maxWidth="26rem"` on both pages) — a 4-field form didn't need the default 640px fullscreen width. All three fixes were live-verified the same way as 19.2, this time including an actually-aborted in-flight request (`page.route(...).abort()`) to prove the network-error path specifically, not just asserted from reading the code.

**Worth remembering as a durable pattern** (see also the new Conventions entries below): a live-verification pass that only exercises the *happy* and *guard-rejected* paths of a new save/edit flow will still miss the *idle* path (nothing changed) and the *transport-failure* path (the request itself never completes) — both need their own explicit check, not just inferred from the other two passing.
...
- **A read-then-write check that must stay atomic (credit limit, or anything similar in the future) belongs inside the same transaction as the write it's guarding, not a pre-check that runs before the transaction opens** — a pre-check lets two concurrent requests each read the same not-yet-updated state and both pass. `checkCustomerCreditLimit()` takes a Prisma client (`tx` or the plain singleton) as its first argument specifically so it can be called from inside a transaction. Fixed Era 15.
- **A create route accepting an idempotency key should verify the matched row's identity before treating a key match as a safe replay, not just the key match alone** — the key is only globally unique in the DB, not scoped to a parent/customer/vendor. Child-resource routes (Payment/PurchasePayment/Return, Era 12.6) already checked the matched row's parent id; the two top-level create routes (Invoice, PurchaseBill) didn't, until Era 15 added the same check (customerId/vendorId, and total once it's known).
- **`PopoverScrim` intercepts clicks (`pointer-events: auto`)** — this was deliberately flipped from `none` in Era 15 after it turned out to be a real bug (a dimmed element underneath still received the click while the popover also closed, the opposite of what the dim visually promises). Don't revert it back to `none` under the assumption that a scrim should be decorative-only; each popover's own outside-click listener still works correctly with `auto`; since the scrim is portaled to `document.body`, it was never inside any popover's own ref to begin with.
- **A return/credit-note line must inherit its per-line values from the traceable parent document line, never accept a client-supplied override for it, whenever a `productId` links the two** — established for `gstRate` early on and extended to `discountPercent` in Era 16 (`rateByProduct`/`discountByProduct` maps in `src/app/api/invoices/[id]/returns/route.ts`, both keyed by `productId`). The client-supplied value is only trusted as a fallback for a custom, non-catalog return line with no invoice line to trace back to. Apply the same pattern to any future per-line value a return needs to carry forward (e.g. if HSN or unit ever became independently editable on an invoice line) rather than trusting the return request body for it.
- **A point-in-time cap checked only at creation (a credit note against `paidAmount`, or anything similarly shaped in the future) must be re-verified whenever the value it was capped against can later change**, not just re-validated on the record being edited in isolation — editing or deleting a Payment doesn't re-run the check that capped the Return created against it, so that re-check has to live in the Payment's own edit/delete path instead. Established Era 19.1 (`newPaidAmount >= existingReturnTotal`, re-verified inside the same transaction on both the invoice-payment `PUT` and the new `DELETE`).
- **Any edit-form Save button needs a dirty-check gate (`useDirty()`), not just a `saving` gate** — without it, clicking Save with nothing changed still re-submits, causing an unnecessary write and (where the route logs one) a spurious `ActivityLog` entry. First established for Rate List Edit (Era 10.4), missed on the new Edit Payment modals until the user's own review caught it (Era 19.3) — check for this on every future edit-form popup, not just create forms (which correctly skip this gate per the existing New/create-form convention in `CLAUDE.md`).
- **Every `fetch()` call inside an async save/delete handler needs a `try/catch`, even when the route itself is reliable** — a thrown network-level rejection (not a 4xx/5xx response body) skips straight past an unwrapped `if (res.ok)` check and leaves whatever `saving`/`deleting` flag gates the UI stuck `true` forever, with no recovery short of a full page reload. Found missing on the invoice detail page's payment handlers (Era 19.3) while the purchase-bill detail page's equivalent handlers already had it — when copying a handler's shape for a new feature, copy its error-handling too, not just its happy path.
