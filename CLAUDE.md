...
- **Do not** assume the invoice/purchase-bill number has a fixed shape or that the sequence is a fixed-index segment — the layout itself is admin-configurable (Settings → Document Numbering, `BusinessSettings.invoiceNumberFormat`/`purchaseBillNumberFormat`, see `NUMBER_FORMATS` in `src/lib/documentNumbering.ts`). Always go through `computeNextNumber()`/`numberFormatDbFilter()`/`findMaxSequence()` to generate or parse a number rather than hand-rolling string splitting. The year segment is always the Indian financial year (Apr-Mar, via `getIndianFinancialYear()`/`formatFinancialYearLabel()`), never the calendar year.
