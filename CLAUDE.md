...
| GET | `/api/gst-filing` | GSTR-style filing package (Sales/Purchase Register, B2B/B2C split, Credit Notes, HSN Summary) for a date range — JSON, `?format=zip`, `?format=xlsx` (2026-08-12, via a dedicated `buildGstFilingWorkbook()` in `src/lib/gstFilingWorkbook.ts` — not the generic `/api/export-xlsx` route, since the filing package needs multiple named sheets rather than one rows→columns table), or `?format=gstr1csv` (2026-08-26, see Features Completed #38). Gated by `requireGstFilingAccess()`: admin, or both `reports_sales`+`reports_purchases` sections |
...
36. **Full-app audit, all 14 phases (2026-08-18 → 2026-08-21)** — see `docs/SCIENCE_HUB_AUDIT_STATE.md`/`docs/SCIENCE_HUB_AUDIT_REPORT.md` for the complete finding-by-finding record; summary of what actually changed in the app (as of this note, still uncommitted working-tree changes — verify via `git log`/`git status` before assuming any specific item has landed on a branch):
