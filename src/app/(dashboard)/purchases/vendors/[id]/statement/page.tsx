...
// running balance = payment debit − bill credit — negative means we owe the vendor.
const bal = (n: number) => `${fmt(Math.abs(n))} ${n < 0 ? "Payable" : "Advance"}`;

interface Party { id: string; name: string; company: string | null; phone: string | null; email: string | null; address: string | null; city: string | null; state: string | null; pincode: string | null; gstin: string | null; }
interface Statement {
  vendor: Party;
...
// A statement has no single `updatedAt` of its own (it's derived live from bills/payments) — so the
// PDF cache key is fingerprinted off the actual rendered numbers instead. Any underlying data change
...
export default function VendorStatementPage() {
  ...
  const { data: statement, loading, error } = useFetch<Statement>(id ? `/api/vendors/${id}/statement?${qs.toString()}` : null);
  ...
  async function handleDownloadPdf() {
    ...
    a.href = url; a.download = `Statement-${statement.vendor.name.replace(/[^a-z0-9]+/gi, "-")}.pdf`;
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      await downloadXlsx(`Statement-${statement.vendor.name}`, "Statement", ["Date", "Particulars", "Debit", "Credit", "Balance"], rows);
    } catch {
  ...
  async function handleShare(channel: "native" | "whatsapp" | "email") {
    setShareOpen(false);
    if (!statement) return;
    const name = statement.vendor.name;

    if (channel === "email") {
      setEmailTo(statement.vendor.email ?? "");
  ...
  async function handleSendEmail(e: React.FormEvent) {
    ...
    try {
      ...
      formData.append("pdf", blob, `Statement-${statement.vendor.name}.pdf`);
      formData.append("to", emails.join(","));
      formData.append("title", `Statement — ${statement.vendor.name}`);
  ...
  return (
    <div className={`page-stack ${styles.pageStack}`}>
      ...
      {pdfPreviewUrl && (
        <PdfPreviewModal
          url={pdfPreviewUrl}
          fileName={`Statement-${statement?.vendor.name.replace(/[^a-z0-9]+/gi, "-") ?? "Vendor"}`}
          title={`Statement${statement ? ` — ${statement.vendor.name}` : ""}`}
      ...
      {statement && (
        <StatementPrintArea
          party={statement.vendor}
          periodLabel={periodLabel}
          openingBalance={statement.openingBalance}
          closingBalance={statement.closingBalance}
          balanceLabel="Payable"
          positiveLabel="Advance"
          negativeLabel="Payable"
          rows={statement.rows}
          settings={settings}
        />
      )}
      <Breadcrumb items={statement ? [
        ...
        { label: statement.vendor.name, href: `/purchases/vendors/${id}` },
        { label: "Statement" },
      ] : [{ label: "Vendors", href: "/purchases/vendors" }]} />

      <div {...animateSection(0, "card")}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}>Account Statement{statement ? ` — ${statement.vendor.name}` : ""}</h2>
