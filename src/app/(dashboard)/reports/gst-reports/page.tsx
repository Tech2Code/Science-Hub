...
// IST-anchored (not the browser's own local/system clock) — a raw `Date.getFullYear()/getMonth()`
// read the wrong calendar day for ~5.5 hours around any date boundary on a non-IST-clocked device,
// which right at the FY rollover (1 April) meant the new financial year could be entirely missing
// from the dropdown and the wrong FY generated by default. Same helpers the server-side FY/date
// logic already uses (documentNumbering.ts/validation.ts), so this page can't independently drift.
function todayStr() {
