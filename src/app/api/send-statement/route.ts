...
// Mirrors /api/send-invoice / /api/send-rate-list. A statement has no fixed document number, so
// the subject/filename are built from a free-text "title" (e.g. "Statement — Acme Traders") instead.
export async function POST(req: Request) {
  const auth = await requireWriteAccess();
  if (!auth.ok) return auth.response;
  const { session } = auth;

  const limit = rateLimit(`send-statement:${session.user.id}`, 20, 15 * 60 * 1000);
  if (!limit.allowed) {
  ...
  try {
    ...
    // Supports a comma-separated list so a statement can go to more than one recipient in a single send.
    ...
    if (/[\r\n]/.test(title) || title.length > 200) {
      return NextResponse.json({ error: "Invalid statement title." }, { status: 400 });
    }
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
          ...
          <p>Please find the <strong>${safeTitle}</strong> attached to this email.</p>
