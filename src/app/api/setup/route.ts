...
export async function POST(request: NextRequest) {
  try {
    ...
    // Products (20 realistic lab supply items)
    const productData = [
    ...
    for (const p of productData) {
      const prod = await prisma.product.create({
        data: {
          name: p.name, sku: p.sku, unit: p.unit, price: p.price, listPrice: p.price, purchasePrice: p.price, gstRate: p.gstRate,
    ...
    // Customers (10 real-world style lab/pharma/education customers)
    const customerRows = [
