...
const MAX_ROWS = 2000; // a manually-curated product catalog is never realistically this long — guards a mis-exported/malicious sheet from tying up CPU/memory on parsing
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Same rate-limit shape as rate-lists/parse-import — this route does synchronous ExcelJS
    // parsing per request with no per-user throttle otherwise.
    const limit = rateLimit(`products-import:${auth.session.user.id}`, 20, 15 * 60 * 1000);
    if (!limit.allowed) {
    ...
    // from tying up this request parsing tens of thousands of rows — a manually-curated product
    // catalog is never realistically this long. +1 keeps room for a header row the parser hasn't detected yet.
